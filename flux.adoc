...
There is another way to subscribe to a stream, which is to call `Mono.get()` or a converter method followed by `get()`. `Flux` has converters `toStream()`, `toList()` and `toMap()`, and `Mono` has `toFuture()`. The `toList()` and `toMap()` converter methods don't actually subscribe to the stream, but they do throw away any control you might have had over the suscription.

WARNING: A good rule of thumb is "**never call get()**". The exception to the rule is in tests: it's useful to be able to block a test to allow results to accumulate.
